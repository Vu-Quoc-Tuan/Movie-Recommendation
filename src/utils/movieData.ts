...
export interface Movie {
  ...
  created_at?: string;
  mood?: string; // From DB - comma-separated mood values like "tense, dark, sad"
}
...
export async function getMovies(
  ...
  try {
    ...
    return (data || []).map(movie => ({
      ...
      // Parse mood field from DB (comma-separated values like "tense, dark, sad")
      mood: movie.mood,
      vibes: movie.mood
        ? movie.mood.split(',').map((m: string) => m.trim())
        : ['Emotional'],
