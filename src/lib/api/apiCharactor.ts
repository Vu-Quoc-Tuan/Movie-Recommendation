...
export async function embeddingAPI(userInput: string): Promise<{embedding: number[]} | null> {

  try {
    
    const response = await fetch('https://clovastudio.stream.ntruss.com/v1/api-tools/embedding/v2/', {
      method: 'POST',
      headers: {
