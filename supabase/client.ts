...
if (!SUPABASE_PROJECT_ID) {
  throw new Error('SUPABASE_PROJECT_ID not found. Set it in src/utils/supabase/info.ts or as an env var.');
}

if (!SUPABASE_PUBLIC_ANON_KEY) {
  throw new Error('SUPABASE_PUBLIC_ANON_KEY not found. Set it in src/utils/supabase/info.ts or as an env var.');
}
